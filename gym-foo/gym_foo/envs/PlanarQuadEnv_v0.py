...
class PlanarQuadEnv_v0(gazebo_env.GazeboEnv):
    ...
    def __init__(self):
        ...
        gazebo_env.GazeboEnv.__init__(self, "PlanarQuadEnv_v0.launch")
        self.vel_pub = rospy.Publisher('/mobile_base/commands/velocity', Twist, queue_size=5)
        self.effort_pub = rospy.Publisher('/.../commands/effort', JointState, queue_size=5)
        ...
        high_state = np.array([5., 2.5, 5, 2.5, 5., np.pi / 6])
        high_obsrv = np.array([5., 2.5, 5, 2.5, 5., np.pi / 6] + [5*2] * self.num_lasers)
        high_action = np.array([3, 3])

        self.state_space = spaces.Box(low=-high_state, high=high_state)
        self.observation_space = spaces.Box(low=np.concatenate((-high_state, np.array([0]*self.num_lasers)), axis=0), high=high_obsrv)
    ...
    def step(self, action):
        ...
        cmd_effort = JointState()
        self.effort_pub.publish(cmd_effort)
        ...
        if self.reward_type == 'hand_craft':
            ...
            reward = self.brsEngine.evaluate_ttr(obsrv[:6])
        done = False
